...
namespace Web
{
    public class WebRepository : IWebRepository
    {
        ...
        public async Task<IEnumerable<Racecar>> SearchRacecarsByCarModel(CarModel model)
        {
            return await _db.Racecars.Where(r => r.Model == (model)).ToListAsync();
        }
        ...
        public async Task<IEnumerable<Racecar>> SearchRacecarsByStatus(CarStatus status)
        {
        ...
        public async Task<IEnumerable<Race>> SearchRacesByCategory(RaceCategory category)
        {
            return await _db.Races.Where(r => r.Category == (category)).ToListAsync();
        }

        public async Task<IEnumerable<Race>> SearchRacesByDate(string date)
        {
            return await _db.Races.Where(r => r.Date == DateTime.Parse(date)).ToListAsync();
        }

        public async Task<IEnumerable<Race>> SearchRacesByWinner(Driver driver)
        {
            return await _db.Races.Where(r => r.Winner == driver).ToListAsync();
        }

        public async Task<IEnumerable<Race>> SearchRacesByParticipant(Driver driver)
        {

            var Races = await _db.Races.Include(p => p.Participants).ToListAsync();
            return Races.Where(r => r.Participants.Contains(driver));
